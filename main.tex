...
\lstset{ %
	...
	otherkeywords={lemma, forall, Cons, Nil, goal, exists, string, float, option, list, int /\\},           % if you want to add more keywords to the set
...
The first people to acknowledge, even if Umberto Eco finds it distasteful,  are my supervisors: Dr Rosemary Monahan and Dr James F. Power. 
Their guidance and expertise were critical to the development of this project.
I thank them for giving me the opportunity to conduct this research.

Secondly, I must thank the broader staff and postgraduate students of the Department of Computer Science.
They have made the past two years very enjoyable ones.
Special thanks to members of the POP group, past or present,
who have been a valuable source of encouragement and domain-specific knowledge. 
Marie deserves a special mention for proof-reading this thesis.

It is conventional for developers of portfolio solvers to acknowledge the development effort associated with their constituent tools, and I must do the same. 
In particular, I am indebted to the \why~system, on which this work relies.
...
\textbf{N.B.} In the text, to distinguish references to Machine Learning (ML) from references to the \texttt{ML} programming language, the latter is typeset in \texttt{monospace} font.
