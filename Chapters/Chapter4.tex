...
This method of verification is particularly useful when \why~is run in batch mode from the command line.
...
\[
	contrib_{norm}(A) = \frac{contrib(A)}{\sum_{i=1}^{|S|} contrib(S_i)} \times \frac{100}{1}
\]
\begin{figure}
\centering
...
Nevertheless, including multiple versions of solvers improves prediction accuracy and ultimately makes \where~a more flexible tool for the user.     
...
\begin{table}
	\caption[Relative importance of features for \where]{Relative importance of each feature in the \where~Random Forest model}
