...
More details about the type of prediction task chosen for the evaluation of ML algorithms is given in Sec. \ref{sec:reg-class}, with an introduction to a number of theoretical strategies chosen for comparison in Sec. \ref{sec:strategies}. A more detailed introduction to the six prediction algorithms (introduced previously in Sec. \ref{sub:lrsvmmml} and Table \ref{table:algorithms}) is given in Sec. \ref{pred:choosing} before the results of their comparison is discussed. We end this chapter with a more detailed look at the model chosen for use in the actual implementation of \where~.
...
\begin{table}
	\caption[Results of running 8 solvers on the example \why~programs]{Results of running 8 solvers on the example \why~programs.  Also included is a theoretical solver  \textsf{Choose Single}, which always returns the best answer in the fastest time.}
...
%\begin{table}
%	\centering
%	\caption[Number of trivial and hard \why~verification tasks]{Number of trivial and hard \why~verification tasks}
...
%It is more natural to select separate solvers on a per-goal basis through the \why~IDE which breaks each WhyML program into theories and goals automatically. 
When using the \why~IDE, it is more natural to prove programs by applying solvers to individual goals.
...
3 files 55 theories and 206 goals fall under the trivial category.
Again, this table provides an interesting insight into the behaviour of all solvers: 3 of the 5 files proved by veriT are trivially provable by all solvers (which may explain the short amount of time taken, on average, to prove the 5 files). 
The second row of this table shows the number of \textit{Hard} file, theories and goals. 
...
211 goals fall under this category.

The breakdown of the number of files, theories and goals which \textit{only} a single solver is able to prove is given in Table \ref{table:unique}.
The good performance of Alt-Ergo version 1.01 is obvious: 6 complete files can only be proved in their entirety by the solver. 
A higher number of goals require the use of CVC4 in order to be proved, however.
33 goals (compared to 25 for the newer version of Alt-Ergo) can \textit{only} be proved by this solver.
...
Should a solver which returns an answer of \textit{Valid} in 5 seconds be seen as ``worse'' than one that returns \textit{Unknown} in 0.5 seconds? 
...
We reject this approach rejected as some benefits associated with portfolio-solving are lost, since if the PO is misclassified, the performance of the portfolio solver suffers severely.
...
With 8 SMT solvers there are 8! (or 40,320) rankings which is far too many to be reasonable for a classification task. 
We rejected this approach because many in our dataset of 1048 POs many of these rankings could not possibly be represented.
...
Table \ref{table:cost} lists the results of executing the 8 solvers on the goal, as measured using the method described in Sec. \ref{sec:dependant}, with a time limit of 10 seconds. 
...
Decision trees \cite{DecisionTrees} recursively construct a binary tree by  splitting the data on the feature and threshold which create the most distinct partitions.
...
By using this constraint, non-leaf nodes are converted into leaves if, when split, they would have produced leaves of size less than 5. 
...
In Table \ref{table:predselection} we list the cross-validation results for all 6 trained prediction models (with discretised and/or weighted variants where applicable).
...
The Mean Average Error is a good measure of overall ranking accuracy, but it fails to take into account that in many use cases -- including \where~-- it is often the highest-ranking items that are the most important to predict accurately. 
...
The regression error may be treated with a note of caution, however, as the relative ranking of solvers -- which we argue is the most important prediction -- is not explicitly taken into account.
...
\begin{table}
	\caption[Relative feature importances for \where]{Relative importance for each feature in the \where~Random Forest model}
...
They score highest for three out of 5 criteria (shown in bold with an asterisk) and have generally good scores in the others.
...
Before discussing the implementation of \where~in the next chapter, we shall investigate the properties of the Random Forest model after it was fit on the entire training dataset.
...
This chapter began by motivating the use of portfolio-solving in the \why~system by showing that more files, theories and goals can be proven by using the best-performing solver on a per-goal basis.
...
We will encounter these concepts again during the evaluation of \where~on test data in Chapter \ref{Evaluation}.
