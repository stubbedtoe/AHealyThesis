...
Verification POs undergo two processes: feature vectors based on syntax are derived statically, and the result of proving the PO using several SMT solvers are recorded. 
...
The same portion be used to train the \where~tool in Chapter \ref{Implementation}.
...
\begin{itemize}
	...
	\item \textbf{Z3} is the SMT solver developed at Microsoft Research. The source code has be available in 2012 and it has been open-source since 2015. \where~uses two versions: 4.3.2 and 4.4.1 
\end{itemize}
...
Proof obligations are generated from the goals and lemmas only, while the axioms and predicated provide the context for these POs to be proved by the SMT solver.
...
This code is one of the 13 \textit{Why} proof obligation goals from the \textbf{edit\_distance.mlw} file in the \textit{Why3} examples directory. 
...
A representation of the parse tree from this lemma is shown in Fig. \ref{fig:tree}. The reader will note that the zero-arity function \textit{Nil} is a leaf node in the tree, while all other functions (\textit{Cons, =, ++, length}) and operators (\textit{and}) have an arity of either 1 or 2. 
...
Efforts to accurate measure execution time are hampered by random errors introduced by the experimental environment.
...
As each solver execution can be quite expensive in terms of time (a worst case scenario for one measurement of each PO: 1048 (POs) $\times$ 8 (solvers) $\times$ 10 seconds (time limit) $\approxeq$ 23.3 hours of computation time), a small number of initial measurements (5) were made.
From these sample measurements, the mean execution time $\bar{x}$ and standard deviation $s$ were computed. 
To determine the number of measurements ($n$) required for a statistically-significant sample, we assume that the random errors have a $t$ distribution. 
...
where $z$ is a value from the $t$ distribution (similar in shape to a Gaussian distribution) which is used to model the measurement error. 
...
We allow the computed mean value to be within 7\% of the actual mean (i.e. 7 degrees of freedom or an allowed error of $\pm$3.5\%). Thus for the equations \ref{eq:c1} to \ref{eq:n}, we take $e = 0.035$.  \\ 
\\
...
The 5 sample measurements of CPU time spent by Alt-Ergo (to return an answer of \textit{Unknown}):
...
give a sample mean ($\bar{x}$) and sample standard deviation ($s$) of 0.133 and 0.006 respectively. 
Consulting a table for the $t$ distribution, we find that when $a=0.1$ and we require 7 degrees of freedom, $t_{0.95;7} = 1.895$.
...
\end{equation}    
Therefore, we need to make 6 measurements to be assured that there is a 90\% chance that the true value is within this $\pm3.5\%$ interval.

For completeness, the extra measurement yielded a value of 0.135; making Alt-Ergo's time spent on CPU solving \textit{first\_last} the mean of the 6 measurements: 0.134 seconds.
...
\begin{itemize}
	\item[\textbf{Valid}] The goal is proved in the given context.
	\item[\textbf{Invalid}] The prover knows the goal cannot be proved.  
	\item[\textbf{Unknown}] The prover has stopped its search.
	\item[\textbf{Timeout}] The prover has reached the time limit.
	\item[\textbf{Failure}] An error has occurred. 
\end{itemize}
...
If Z3 is given a time limit of 60 seconds, only 125 more POs can proved; giving a total of 632.  
...
\why, for example, has a default timeout value of 5 seconds.
...
From this observation we deduce that the likelihood of a \textit{Timeout} answer potentially turning into a \textit{Valid/Invalid} answer given more time is minimal.  
...
The choices made in regard to the SMT solvers was dictated to a large extent by \why~and its selection of drivers. 
...
\begin{itemize}
	\item the time each of the eight solvers takes to return an answer when given a time limit value of 10 seconds
	\item each solver's response (\textit{Valid/Timeout/etc.}) for the same time limit
	\item a feature vector consisting of the 28 structural and syntactic metrics listed in Fig. \ref{fig:tree} 
\end{itemize}   
