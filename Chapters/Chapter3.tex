...
Fig. \ref{fig:confidence} shows measurement error modelled as a standard distribution. 
The probability that \textit{x}, the actual value being measured (i.e. the execution time for each solver to return an answer for a particular PO), is within the confidence interval ($c_1, c_2$) is $1-a$. 
A value of 0.1 was chosen for \textit{a}, meaning we can be 90\% confident that the actual value of \textit{x} is between $c_1$ and $c_2$, with an allowed error of $\pm$3.5\%.   
...
\end{itemize}
Fig. \ref{fig:barcharts} shows the relative amount of Valid / Unknown / Timeout / Failure answers from the eight SMT solvers (when given a timeout of 60 seconds). 
Note that no tool returned an answer of Invalid for any of the 1048 proof obligations.
Often, goals that cannot be proved Valid or Invalid have been inductively defined and usually require the use of an interactive theorem prover. 
...
This value was chosen as an upper limit, since a timeout value of 60 seconds is not realistic for most software verification scenarios.  
