...
% explain rank-1 polymorphism here
...
While proof scripts can be thought of as being similar to conventional programs ( describing \textit{how} something is done), formal specifications are quite different: providing a precise description of \textit{what needs} to be done.
...
\begin{table}
	\caption[Summary of ML algorithms encountered and evaluated]{Summary of ML algorithms encountered and evaluated}
	\begin{adjustbox}{angle=90}  
		\begin{tabularx}{0.9\textheight}{@{}Y|Y|Y|Y@{}}
			...
			\midrule
			Decision Trees & Multi-output, multi-label classification \& regression problems & Recursively partition data by thresholding features based on maximising information gain & Classification and regression of C programs for use by model checkers \cite{MUX,DPVZ15:CAV} \\
			\midrule
			Random Forests & Can be used on the same problems as Decision Trees & Multiple Decision Trees which use a subset of features and/or data. Designed to minimise overfitting & Classification and regression of C programs for use by model checkers \cite{MUX,DPVZ15:CAV} \\
			\midrule
