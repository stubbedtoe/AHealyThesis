...
print important
with open('table_4_6_feature_importances.txt','w') as out:
...
print('Mean time for no pre-solver:')
print(test.apply(lambda ser: common.time_to_valid( ser['rf'],
			test.ix[ser['key'], common.TIMES+common.RESULTS] ), axis=1).mean())

# before counting their results
#what about the case where Alt-Ergo solves but ser['rf'][0] does not?
results['Where4 (PS)'] = {o : test.apply(lambda ser:
	fun(take_better(test.ix[ser['key'], common.REV_MAP[ser['rf'][0]]+' result'], test.ix[ser['key'], 'Alt-Ergo-1.01 result'] )), axis=1).sum()
	for o, fun in output.iteritems()}

results['Where4 (no PS)'] = {o : test.apply(lambda ser:
...
results.to_csv('fig_6_1_data.csv')
print results
...
# alternative get from old data
get_time = DataFrame.from_csv('fig_6_3_data.csv')
